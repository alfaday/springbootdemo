...
public class Server {
    ...
    private void init(){
        ...
        try {
            ...
            //绑定端口
            servSocketChannel.socket().bind(new InetSocketAddress(Constant.port));
    ...
    private void listen() {
        while(true){
            try{
                selector.select();
                System.out.println("select done:" + new Date());
    ...
    private void handleKey(SelectionKey key)
        ...
        try{
            if(key.isAcceptable()){
                ...
                System.out.println("accept sus:" + new Date());
                channel.configureBlocking(false);
                channel.register(selector, SelectionKey.OP_READ);
            }
            else if(key.isReadable()){
                System.out.println("ready to read:" + new Date());
